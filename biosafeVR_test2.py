...
def save(hexagons_new, hexagons_old):
    """
    Function to test the code separately from the Virtual River.
    """
    root_path = os.path.dirname(os.path.realpath(__file__))
    test_path = os.path.join(root_path, 'test_files')
    with open(os.path.join(test_path, 'hexagons_reference.geojson'), 'w') as f:
        dump(hexagons_old, f, sort_keys=True, indent=2)
    with open(os.path.join(test_path, 'hexagons_intervention.geojson'), 'w') as f:
         dump(hexagons_new, f, sort_keys=True, indent=2)        
    return
...
def main():
    ...
    save(hexagons_new, hexagons_old)
