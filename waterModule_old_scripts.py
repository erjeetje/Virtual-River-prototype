...
class Water():
    ...
    def water_level_graph(self):
        ...
        if len(self.dike_levels) > 0:
            label = "dike levels"
            new_water_levels = ax.plot(self.x_hexagons, self.dike_levels, label=label)
...
def water_levels(hexagons):
    """
    This function return the water level based on the water levels stored in
    the hexagon features. Looks at the floodplain and main channel hexagons.
    
    Currently not called in Virtual River.
    """
...
def water_levels_new(hexagons):
    """
    This function return the water level based on the water levels stored in
    the hexagon features. Looks at the main channel hexagons.
...
def grid_to_hex(hexagons, grid):
    """
    This function gets the water levels from the model and stores them in the
    appropriate hexagon feature.
...
def plot_water_levels(xvals, yvals, label="", turn=0, fig=None, ax=None,
                      x_mani=-30):
    """
    This function creates a plot of the water levels.
...
def plot_water_levels_bspline(xvals, yvals, label="", turn=0, fig=None, ax=None,
                      x_mani=-30):
    """
    This function creates a plot of the water levels.
    ...
    for i in range(len(yvals)):
        xvals.append(i * x_mani + 3500)
    xnew = np.linspace(min(xvals),max(xvals),150)
    spl = make_interp_spline(sorted(xvals), yvals, k=3) #BSpline object
    ...
    """
    xnew = np.flip(xnew, 0)
    ax.plot(xnew, power_smooth, label=label)
    ax.legend(loc='upper right')
    plt.show
    return fig, ax
...
def get_channel(hexagons):
    """
    This function returns the main channel cells of the game board.
...
def get_crosssection(hexagons, grid, save=False):
    """
    This function adds crosssections to the grid based on hexagon locations.
...
def grid_river_axis_old(hexagons, grid, save=False):
    """
    ...
    This version of the function finds only one cell for each crosssection
    (cell columns).
    """
...
def grid_river_axis_water_levels_old(grid, model, save=False):
    #s1 = model.get_var('s1')
    x_output = []
    s1_output = []
    for feature in grid.features:
...
def grid_waterlevels(grid, model, save=False):
    """
    This function determines the water levels of the entire crosssections,
...
def main():
    ...
    hexagons = load(turn=turn)
    face_grid = load_grid(turn=turn)
    face_grid = determine_grid_river_axis(hexagons, face_grid)
    x_output, water_levels = grid_river_axis_water_levels(face_grid, save=True)
    ...
    face_grid = determine_grid_river_axis(hexagons, face_grid)
    x_output, water_levels = grid_river_axis_water_levels(face_grid)
    fig, ax = plot_water_levels(x_output, water_levels,
    ...
    x_output = get_river_length(water_levels)
    fig, ax = plot_water_levels_bspline(x_output, water_levels,
                                label="water levels turn 0", turn=turn,
                                x_mani=-250)
    hexagons = load(turn=1)
    face_grid = load_grid(turn=1)
