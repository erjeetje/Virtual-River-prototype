...
def set_terrain_type(api_key, hexagons, updated_hex=[]):
    """
    ...
    land = []
    new_land_hexagons = []
    # this updated_hex implementation will not work properly, fix when there is
    # the possibility to properly debug.
    for feature in hexagons.features:
        if feature.properties["ghost_hexagon"]:
            continue
        if (feature.properties["water"] and (feature.properties["z_changed"] or
                                             feature.id in updated_hex)):
